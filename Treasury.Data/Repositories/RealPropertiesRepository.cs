...
namespace Treasury.Data.Repositories
{
    public class RealPropertiesRepository : IRealPropertiesRepository
    {
        ...
        private readonly IProvinces _provinces;
        private readonly IMunicipalities _municipalities;
        private readonly IBarangayRepository _barangayRepository;
        private readonly IActualUseCodes _actualUseCodes;
        private readonly IClassificationCodes _classificationCodes;
        ...
        private readonly IRptPreviousAssessment _rptPreviousAssessment;
                                        ...
                                        IRptPreviousAssessment rptPreviousAssessment)
        {
            ...
            _municipalities = municipalities ?? throw new ArgumentNullException(nameof(municipalities));
            _barangayRepository = barangayRepository ?? throw new ArgumentNullException(nameof(barangayRepository));
            _actualUseCodes = actualUseCodes ?? throw new ArgumentNullException(nameof(actualUseCodes));
            _classificationCodes = classificationCodes ?? throw new ArgumentNullException(nameof(classificationCodes));
            _taxpayerTypeRepository = taxpayerTypeRepository ?? throw new ArgumentNullException(nameof(taxpayerTypeRepository));
            _taxpayersRepository = taxpayersRepository ?? throw new ArgumentNullException(nameof(taxpayersRepository));
            _rptPreviousAssessment = rptPreviousAssessment ?? throw new ArgumentNullException(nameof(rptPreviousAssessment));
        }
        ...
        public bool Synchronize(RealPropertiesModel realPropertiesModel)
        {
            using (var scope = new TransactionScope())
            {
                ...
                //RptPreviousAssessmentModel _rptPreviousAssessmentModel = realPropertiesModel.RptPreviousAssessmentModels;
                ...
                else
                {
                    ...
                    taxpayerId = _taxpayersRepository.GetLastInsertedId(null);
                }
                ...
                if (CompleteArpNoExist(completeArpNo))
                {
                    ...
                    //_rptPreviousAssessmentModel.RealPropertiesId = realPropertiesId;
                    ...
                    //_rptPreviousAssessment.Insert(_rptPreviousAssessmentModel);
                }
                else
                {
                    realPropertiesModel.ActualUseCodesModel.Id = actualUseId;
        ...
        public DataTable GetRecordNotExistedPreviousRpt(int rptId)
        {
            ...
            string query = $"SELECT * FROM {viewTableName} WHERE  NOT EXISTS(SELECT {_rptPreviousAssessment.tableName}.real_properties_id FROM {_rptPreviousAssessment.tableName} WHERE {_rptPreviousAssessment.tableName}.real_properties_id = {viewTableName}.real_property_id) AND real_property_id <> @real_property_id";
        ...
        {
            string query = $"SELECT * FROM {viewTableName} WHERE NOT EXISTS(SELECT {_rptPreviousAssessment.tableName}.real_properties_id FROM {_rptPreviousAssessment.tableName} WHERE {_rptPreviousAssessment.tableName}.real_properties_id = {viewTableName}.real_property_id)";
        ...
        public bool InsertWithPreviousAssessments(RealPropertiesModel realPropertiesModel)
        {
            using (var scope = new TransactionScope())
            {
                ...
                foreach (var model in realPropertiesModel.RptPreviousAssessmentModels)
                {
                    ...
                    _ = _rptPreviousAssessment.Insert(model);
                }
        ...
        public bool UpdateWithPreviousAssessements(RealPropertiesModel realPropertiesModel)
        {
            ...
            {
                Update(realPropertiesModel);
                _ = _rptPreviousAssessment.DeleteByRealPropertyId(realPropertiesModel.Id);
