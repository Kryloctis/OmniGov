...
namespace ACC.Data
{
    public class BankDepositsRepository : IBankDepositsRepository
    {
        ...
        public DataTable GetRecords()
        {
            try
            {
                string query = $"SELECT {tableName}.id,{tableName2}.account_no,{tableName2}.bank_name,{tableName}.reference,{tableName}.date,{tableName}.amount,{tableName}.created_at,{tableName}.updated_at,CONCAT(u1.last_name,', ',u1.first_name,' ',u1.mid_initial) AS createdby,CONCAT(u2.last_name,', ',u2.first_name,' ',u2.mid_initial) AS updatedby FROM {tableName} LEFT JOIN {tableName2} ON {tableName}.banks_id={tableName2}.id LEFT JOIN {tableName3} u1 ON {tableName}.created_by=u1.id LEFT JOIN {tableName3} u2 ON {tableName}.updated_by=u2.id";
        ...
        public DataTable GetRecordsBySearch(int id)
        {
            try
            {             
                string query = $"SELECT {tableName}.id,{tableName2}.account_no,{tableName2}.bank_name,{tableName}.reference,{tableName}.date,{tableName}.amount,{tableName}.created_at,{tableName}.updated_at,CONCAT(u1.last_name,', ',u1.first_name,' ',u1.mid_initial) AS createdby,CONCAT(u2.last_name,', ',u2.first_name,' ',u2.mid_initial) AS updatedby FROM {tableName} LEFT JOIN {tableName2} ON {tableName}.banks_id={tableName2}.id LEFT JOIN {tableName3} u1 ON {tableName}.created_by=u1.id LEFT JOIN {tableName3} u2 ON {tableName}.updated_by=u2.id WHERE {tableName}.banks_id='{id}'";
