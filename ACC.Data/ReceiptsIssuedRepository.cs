    ...
    public class ReceiptsIssuedRepository:IReceiptsIssuedRepository
    {
        ...
        public Dictionary<string, string> GetRecordByID(int Id)
        {
            ...
            try
            {
                ...
                string query = $"SELECT * FROM {tableName} LEFT JOIN {tableName5} ON {tableName}.receipts_id={tableName5}.id WHERE {tableName}.id = @id";
        ...
        public DataTable GetRecords(int id)
        {
            try
            {
                string query = $"SELECT * FROM {tableName5} LEFT JOIN {tableName4} ON {tableName5}.accountable_forms_id={tableName4}.id WHERE {tableName5}.id NOT IN (SELECT {tableName}.receipts_id FROM {tableName} WHERE {tableName}.collecting_officers_id='{id}' AND IF(IFNULL({tableName}.is_returned,0)>0,1,0)=0)";
        ...
        public DataTable GetRecords(string coid, string formid)
        {
            try
            {
                string query = $"SELECT * FROM {tableName5} LEFT JOIN {tableName} ON {tableName}.receipts_id={tableName5}.id LEFT JOIN {tableName4} ON {tableName5}.accountable_forms_id={tableName4}.id WHERE {tableName}.collecting_officers_id='{coid}' AND {tableName5}.accountable_forms_id='{formid}' AND IF(IFNULL({tableName}.is_returned,0)>0,1,0)=0 AND IF({tableName5}.receiptsto={tableName5}.last_issued,true,false)=false";
        ...
        public bool Insert(ReceiptsIssuedModel entity)
        {
            try
            {
                ...
                string query = $"INSERT INTO {tableName} (receipts_id,collecting_officers_id,date_issued) VALUES (@receipts_id,@collecting_officers_id,@date_issued)";
        ...
        public bool Update(ReceiptsIssuedModel entity)
        {
            try
            {
                ...
                string query = $"UPDATE {tableName} SET receipts_id=@receipts_id,collecting_officers_id=@collecting_officers_id,date_issued=@date_issued WHERE id = @id";
                return _dbGenericCommands.ExecuteNonQuery(query, parameters);
            }
            catch (Exception)
            {
