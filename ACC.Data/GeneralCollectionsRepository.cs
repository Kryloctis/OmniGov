...
namespace ACC.Data
{
    class GeneralCollectionsRepository:IGeneralCollectionsRepository
    {
        ...
        private readonly string tableName15 = "collector_reports";
        ...
        public DataTable GetRecordByData(int Id)
        {
            try
            {
                string query = $"SELECT {tableName}.id,{tableName}.rcd_no,{tableName}.rcd_date,CONCAT({tableName2}.last_name,', ',{tableName2}.first_name,' ',{tableName2}.mid_initial) AS officer,(SELECT SUM({tableName5}.amount) FROM {tableName5} LEFT JOIN {tableName4} ON {tableName4}.payment_collections_id={tableName5}.id LEFT JOIN {tableName3} ON {tableName3}.collector_report_id={tableName4}.collector_report_id WHERE {tableName3}.general_collections_id={tableName}.id) AS colamount,(SELECT IF(COUNT({tableName6}.id)>0,true,false) FROM {tableName6} WHERE {tableName6}.general_collections_id={tableName}.id) AS deposited FROM {tableName} LEFT JOIN {tableName2} ON {tableName2}.id={tableName}.user_id WHERE {tableName}.id='{Id}'";
        ...
        public DataTable GetRecordByForms(int Id)
        {
            try
            {
                string query = $"SELECT {tableName}.id,{tableName9}.id AS formid,{tableName9}.acc_form_no,{tableName9}.acc_form_desc,(SELECT pc.receipt_no FROM {tableName3} gcp LEFT JOIN {tableName5} crp ON gcp.collector_report_id=crp.collector_report_id LEFT JOIN {tableName5} pc ON pc.id=crp.payment_collections_id WHERE gcp.general_collections_id={tableName}.id AND pc.accountable_forms_id={tableName5}.id ORDER BY pc.receipt_no ASC LIMIT 1) AS orfrom,(SELECT pc.receipt_no FROM {tableName3} gcp LEFT JOIN {tableName5} crp ON gcp.collector_report_id=crp.collector_report_id LEFT JOIN {tableName5} pc ON pc.id=crp.payment_collections_id WHERE gcp.general_collections_id={tableName}.id AND pc.accountable_forms_id={tableName5}.id ORDER BY pc.receipt_no DESC LIMIT 1) AS orto,(SELECT SUM(pc.amount) FROM {tableName3} gcp LEFT JOIN {tableName5} crp ON gcp.collector_report_id=crp.collector_report_id LEFT JOIN {tableName5} pc ON pc.id=crp.payment_collections_id WHERE gcp.general_collections_id={tableName}.id AND pc.accountable_forms_id={tableName5}.id) AS total FROM {tableName} LEFT JOIN {tableName3} ON {tableName}.id={tableName3}.general_collections_id LEFT JOIN {tableName4}.collector_report_id={tableName3}.collector_report_id LEFT JOIN {tableName5} ON {tableName5}.id={tableName4}.payment_collections_id LEFT JOIN {tableName9} ON {tableName5}.accountable_forms_id={tableName9}.id WHERE {tableName}.id='{Id}' GROUP BY formid";
        ...
        public DataTable GetRecordByDeposits(int Id)
        {
            try
            {
                string query = $"SELECT {tableName}.id,{tableName17}.bank_name,{tableName17}.account_no,{tableName16}.reference,{tableName16}.amount FROM tableName LEFT JOIN {tableName16} ON {tableName}.general_collections_id={tableName}.id LEFT JOIN {tableName7} ON {tableName16}.bank_deposits_id={tableName7}.id WHERE {tableName}.id='{Id}'";
